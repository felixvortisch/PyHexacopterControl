...
def export_rocket_ode_model() -> AcadosModel:
    ...
    # tau Command from Motormatrix K
    values_TAU_CMD = [	[6.6808736e-08, 1.34e-07, 6.68e-08, -6.6808736e-08, -1.33617473e-07, -6.6808736e-08],
    				[-1.15805002e-07, 0.0, 1.16e-07, 1.16e-07, 0.0, -1.15805002e-07],
    				[-9.99e-09,1.00e-08, -9.99e-09,1.00e-08,-9.99e-09,1.00e-08]]
    ...
    x    = vertcat(pos, q, vel, omega, int_error_h, setpoint )
    xdot = vertcat(pos_dot, q_dot, vel_dot, omega_dot, int_error_h_dot, setpoint_dot)

    nx = x.size()[0]
    weight_diag = np.ones((nx,)) * 1e-6  # default weight
    weight_diag[0] = 1500.0 #30000000.0   # pos East
    weight_diag[1] = 1500.0   # pos North
    weight_diag[2] = 1900.0#50000000000.0   # altitude
    ...
    weight_diag[10] = 1.0 
    weight_diag[11] = 1.0 
    weight_diag[12] = 100.0  
    
    weight_diag[13] = 100.0  
    ...
    #omega_dot = J_inv @ (TAU_CMD @ (u * u) - cross(omega, (J @ omega)))  #Gyroscopic Equation Newton
